...
const AuthRequest = axios.create({
  baseURL: ApiContants.BACKEND_API.BACKEND_API_URL,
});

const register = async (user) => {
  if (!user?.username || !user?.email || !user?.password) {
    return { status: false, message: "Porfavor, rellene todos los campos" };
  }
  try {
    let requestBody = {
    ...
    console.log(error);
    return { status: false, message: "¡Oh! Algo salio mal" };
  }
};
